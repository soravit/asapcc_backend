...
/*test*/
    ...
    static updateJob({job_note='',job_service_point_code_confirm='',job_appoint_confirm_datetime='',servicetask1='',servicetask2='',servicenote='',custcare1='',custcare2='',custcare3='',custcarenote='',car_odo_mile='',job_callcenter_note='',job_status='',final_job_id=''}){
        ...
        return db.execute('UPDATE asapcc_job_main SET job_note = ?, job_service_point_code_confirm = ?, job_appoint_confirm_datetime = ? WHERE final_job_id = ?',[job_note,job_service_point_code_confirm,job_appoint_confirm_datetime,final_job_id])
    }
    ...
    static updateservicepointdatainjobtable({final_job_id='',service_point_name='',branch_name='',full_address='',amphor_name_th='',province_name_th='',post_code='',telephone='',mobiletel='',lattitude='',longtitude='',service_group='',service_code=''}){
         
  
        return db.execute("UPDATE `asapcc_job_main` SET `service_point_name` = ?, `branch_name` = ?, `full_address` = ?, `amphor_name_th` = ?, `province_name_th` = ?, `post_code` = ?, `telephone` = ?, `mobiletel` = ?, `lattitude` = ?, `longtitude` = ?, `service_group` = ?, `job_service_point_code_confirm` = ? WHERE `asapcc_job_main`.`final_job_id` = ?",[service_point_name,branch_name,full_address,amphor_name_th,province_name_th,post_code,telephone,mobiletel,lattitude,longtitude,service_group,service_code,final_job_id])
    }

    
    static updateservicepointdata_confirm_injobtable({final_job_id='',service_point_name='',branch_name=''}){
      
  
        return db.execute("UPDATE `asapcc_job_main` SET `service_point_name_confirm` = ?, `branch_name_confirm` = ? WHERE `asapcc_job_main`.`final_job_id` = ?",[service_point_name,branch_name,final_job_id])
    }
...
static statsearchjob_default({customergroup=''}){ 
    ...
    static statsearchjob_all2({daterange=''}){
        return db.execute('SELECT (SELECT COUNT(*) FROM asapcc_job_main '+daterange+' AND job_status="new") AS NEWJOB,(SELECT COUNT(*) FROM asapcc_job_main   '+daterange+' AND  job_status="onprocess") AS ONPROCESS,(SELECT COUNT(*) FROM asapcc_job_main  '+daterange+' AND job_status="finished") AS FINISHED')
    }

    static statsearchjob_filter({othersearch=''}){
        
        return db.execute('SELECT (SELECT COUNT(*) FROM asapcc_job_main WHERE '+othersearch+' AND job_status="new") AS NEWJOB,(SELECT COUNT(*) FROM asapcc_job_main  WHERE '+othersearch+' AND  job_status="onprocess") AS ONPROCESS,(SELECT COUNT(*) FROM asapcc_job_main WHERE '+othersearch+' AND job_status="finished") AS FINISHED')
    }
    static statsearchjob_filter2({othersearch='',daterange=''}){
        
        return db.execute('SELECT (SELECT COUNT(*) FROM asapcc_job_main WHERE '+othersearch+' '+daterange+' AND job_status="new") AS NEWJOB,(SELECT COUNT(*) FROM asapcc_job_main  WHERE '+othersearch+' '+daterange+' AND  job_status="onprocess") AS ONPROCESS,(SELECT COUNT(*) FROM asapcc_job_main WHERE '+othersearch+' '+daterange+' AND job_status="finished") AS FINISHED')
    }
